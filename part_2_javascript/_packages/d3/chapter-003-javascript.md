---
title: D3.js JavaScript
date: 2016-08-22 14:25:29
...
data函数将数据（[1,2]）绑定到我们的选区（所有的rect）,而enter()函数会循环数据中的每个值执行enter()之后的所有函数，例如：
.append(‘rect’)
.attr(‘x’, function(d) { return d * 20; })
.attr(‘y’, function(d) { return d * 50; })

在循环的数据时，创建rect SVG元素，并设置其x，y值
...
d3.csv函数必须有两个参数，第一个参数是请求的文件名称，第二个参数是一个函数，表示当文件请求完成之后读取文件内容作为第二个方法的参数。
需要注意的是，我们从数据文件里读出来的内容格式往往是string，所以需要类型转换，假设data.csv文件内容为：
x,y
100,100
130,120
80,180
